/**
 * Re-seed the learning_flow table with full multi-card data from learningFlowMap.ts
 * Clears existing rows and inserts the complete multi-card structure.
 */
...
// Full multi-card data from learningFlowMap.ts
const flowData = {
  "alcm": {
    deeper: [
  ...
  "for/teenager": {
    deeper: [
      { label: "Promptolinguistics", href: "/promptolinguistics", description: "The hub of the site. The discipline behind everything." },
      { label: "ALCM", href: "/alcm", description: "The model behind the discipline. Eight axes." },
  ...
  "builders-kids": {
    deeper: [
      ...
      { label: "Drift", href: "/drift", description: "What the kids are protected from" },
    ],
    wider: [
  ...
  "kids-learn": {
    deeper: [
      ...
      { label: "Guardian & Teacher Lens", href: "/for/guardian-teacher", description: "What parents and teachers need to know" },
    ],
    wider: [
  ...
  "math-prompting": {
    deeper: [
      ...
      { label: "Variable Scale Theory", href: "/variable-scale", description: "Formalizing session variables" },
  ...
  "hallucinations": {
    deeper: [
      { label: "Drift", href: "/drift", description: "How hallucinations relate to session drift" },
      { label: "Variable Scale Theory", href: "/variable-scale", description: "The honesty dial and hallucination risk" },
      { label: "What Claude Admitted", href: "/what-claude-admitted", description: "AI's own account of its limitations" },
    ],
    wider: [
      { label: "Taxonomy", href: "/taxonomy", description: "Which AIs hallucinate more" },
      { label: "Road Protocol", href: "/road-protocol", description: "How to set up sessions that catch hallucinations" },
      { label: "Five Rules", href: "/rules", description: "Rule 2: Honesty — catching the lie" },
    ],
    simpler: [
      { label: "Everyday Lens", href: "/for/everyday", description: "Hallucinations in plain language" },
      { label: "Child Lens", href: "/for/child", description: "When the AI makes things up — explained for kids" },
    ],
  },
  "three-voices": {
    deeper: [
      { label: "Promptolinguistics", href: "/promptolinguistics", description: "The linguistic layer of voice selection" },
      { label: "ALCM", href: "/alcm", description: "How voice selection maps to the control model" },
    ],
    wider: [
      { label: "Living Lexicon", href: "/lexicon", description: "The vocabulary of the three voices" },
      { label: "Frameworks", href: "/frameworks", description: "Frameworks that use voice selection" },
      { label: "Flower Presets", href: "/flower-presets", description: "Pre-built voice configurations" },
    ],
    simpler: [
      { label: "Five Rules", href: "/rules", description: "The foundation before the voices" },
      { label: "Everyday Lens", href: "/for/everyday", description: "The three voices in plain language" },
    ],
  },
  "user-governance": {
    deeper: [
      { label: "ALCM", href: "/alcm", description: "The control model that governance implements" },
      { label: "Promptolinguistics", href: "/promptolinguistics", description: "The language layer of governance" },
      { label: "EU AI Act", href: "/eu-ai-act", description: "Institutional governance as complement" },
    ],
    wider: [
      { label: "Road Protocol", href: "/road-protocol", description: "Governance in session practice" },
      { label: "Dual Strategy", href: "/dual-strategy", description: "User + institutional governance together" },
      { label: "Framework Families", href: "/frameworks", description: "The governance toolkit" },
    ],
    simpler: [
      { label: "Five Rules", href: "/rules", description: "The simplest governance layer" },
      { label: "Everyday Lens", href: "/for/everyday", description: "Governance in plain language" },
    ],
  },
  "dual-strategy": {
    deeper: [
      { label: "User-Side Governance", href: "/user-governance", description: "The bottom-up half of the strategy" },
      { label: "EU AI Act", href: "/eu-ai-act", description: "The top-down half of the strategy" },
    ],
    wider: [
      { label: "Road Protocol", href: "/road-protocol", description: "Where the strategy becomes practice" },
      { label: "Framework Families", href: "/frameworks", description: "The tools for both sides" },
      { label: "Citizen Researcher", href: "/citizen-researcher", description: "The user-side research methodology" },
    ],
    simpler: [
      { label: "Five Rules", href: "/rules", description: "The simplest version of the strategy" },
      { label: "Everyday Lens", href: "/for/everyday", description: "The strategy in plain language" },
    ],
  },
  "gallantry-ai": {
    deeper: [
      { label: "ALCM", href: "/alcm", description: "The control model at the heart of GallantryAI" },
      { label: "Promptolinguistics", href: "/promptolinguistics", description: "The discipline GallantryAI is built on" },
      { label: "Field Papers", href: "/field-papers", description: "The research record" },
    ],
    wider: [
      { label: "The Builder", href: "/builder", description: "Who built this" },
      { label: "Builder Origin", href: "/builder-origin", description: "Where it started" },
      { label: "Open Door", href: "/open-door", description: "The invitation to join" },
    ],
    simpler: [
      { label: "Five Rules", href: "/rules", description: "The simplest entry point" },
      { label: "Everyday Lens", href: "/for/everyday", description: "GallantryAI in plain language" },
    ],
  },
  "field-papers": {
    deeper: [
      { label: "Citizen Researcher", href: "/citizen-researcher", description: "The methodology behind the papers" },
      { label: "Research Hub", href: "/research-hub", description: "All research in context" },
    ],
    wider: [
      { label: "Counter Arguments", href: "/counter-arguments", description: "The honest criticisms" },
      { label: "What Claude Admitted", href: "/what-claude-admitted", description: "Primary source data" },
      { label: "The Builder", href: "/builder", description: "Who wrote the papers" },
    ],
    simpler: [
      { label: "Five Rules", href: "/rules", description: "The foundation the papers support" },
      { label: "Everyday Lens", href: "/for/everyday", description: "The research in plain language" },
    ],
  },
};

// Clear existing rows and re-insert
await db.execute("DELETE FROM learning_flow");

let count = 0;
for (const [pageSlug, data] of Object.entries(flowData)) {
  await db.execute(
    "INSERT INTO learning_flow (pageSlug, deeperLinks, widerLinks, simplerLinks) VALUES (?, ?, ?, ?)",
    [pageSlug, JSON.stringify(data.deeper), JSON.stringify(data.wider), JSON.stringify(data.simpler)]
  );
  count++;
}

console.log(`✅ Re-seeded ${count} pages with full multi-card learning flow data`);

// Verify spot check
const [rows] = await db.execute("SELECT pageSlug, deeperLinks FROM learning_flow WHERE pageSlug = 'for/guardian-teacher'");
const row = rows[0];
const deeper = JSON.parse(row.deeperLinks);
console.log(`✅ guardian-teacher deeper cards: ${deeper.length} (expected 2)`);
