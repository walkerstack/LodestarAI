...
The owner accesses Studio at `/studio` using password: **Hudson2021!**
...
- Studio CMS at `/studio` — password login working (Hudson2021!)
...
- Checks `input.password` against `ENV.studioPassword` (defaults to "Hudson2021!")
...
**ENV values needed:**
- `STUDIO_PASSWORD` — defaults to "Hudson2021!" if not set
...
2. **Password is Hudson2021!** — do not change it without explicit instruction.
